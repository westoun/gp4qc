...
from gates import Gate, Oracle, Identity
from .fitness import Fitness
from .params import FitnessParams, default_params
from .utils import count_gates, count_gate_calls


class DirectQAFitness(Fitness):
    def __init__(self, params: FitnessParams = default_params) -> None:
        self.params = params

        assert params.classical_oracle_count > 0, (
            "If the DirectQAFitness function is chosen, the "
            "classical_oracle_count of fitness params has to be specified."
        )
    ...
    def evaluate(
        ...
        if len(errors) > 0:
            ...
            fitness_score = (
                count_gate_calls(chromosome, Oracle)
                / self.params.classical_oracle_count
                + count_gates(chromosome) / 100000
            )
